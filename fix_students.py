...
# Correct 45 students with proper dates and full level names
students = [
...
print("Step 1: Delete ALL students...")
resp = requests.get(BASE_URL, headers=headers)
all_students = resp.json()
print(f"Found {len(all_students)} to delete")

for s in all_students:
    del_url = f"{BASE_URL}?id={s['id']}"
    try:
...
print("\nStep 2: Insert 45 correct students...")
success = 0
for student in students:
    ...
    try:
        ...
        if resp.status_code in [200, 201]:
            success += 1
            print(f"[{success}] {student['name']} - {student['level']} - {student['date']}")
    except Exception as e:
        print(f"Error: {student['name']}")
