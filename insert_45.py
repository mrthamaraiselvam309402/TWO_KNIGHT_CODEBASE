...
print("Inserting 45 student records...")
...
for student in students:
    ...
    try:
        ...
        if resp.status_code in [200, 201]:
            success += 1
            print(f"[{success}] {student['name']} - {student['level']} - Rs.{student['fee']}")
        else:
            print(f"ERROR {student['name']}: Status {resp.status_code} - {resp.text[:100]}")
