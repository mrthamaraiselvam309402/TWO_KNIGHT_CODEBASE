  ...
  function generateReportPDFContent() {
    ...
    if (allCoaches && allStudents) {
      ...
      const html = '<!DOCTYPE html>' +
        ...
        '<style>body{font-family:Segoe UI,sans-serif;padding:40px;max-width:800px;margin:0 auto}h1{color:#dca33e;text-align:center;margin-bottom:4px}h2{color:#333;border-bottom:2px solid #dca33e;padding-bottom:8px;margin-top:30px}table{width:100%;border-collapse:collapse;margin:16px 0}th,td{padding:12px;text-align:left;border-bottom:1px solid #ddd}th{background:#f5f5f5}.print-btn{background:#dca33e;color:#000;padding:12px 24px;border:none;border-radius:8px;cursor:pointer;margin:20px 0}@media print{.print-btn{display:none}}</style></head>' +
        '<body>' +
        '<h1>♟ Chesskidoo Academy</h1>' +
        '<p style="text-align:center;color:#666">Financial Report - ' + date + '</p>' +
        '<button class="print-btn" onclick="window.print()">🖨️ Print / Save as PDF</button>' +
        '<h2>Financial Overview</h2>' +
        '<table><tr><th>Metric</th><th>Value</th></tr>' +
        '<tr><td>Total Students</td><td>' + totalStudents + '</td></tr>' +
        '<tr><td>Paid</td><td>' + paidStudents + '</td></tr>' +
        '<tr><td>Due</td><td>' + dueStudents + '</td></tr>' +
        '<tr><td>Total Monthly Revenue</td><td>₹' + totalRevenue.toLocaleString() + '</td></tr></table>' +
        '<h2>Coach Revenue Breakdown</h2>' +
        '<table><tr><th>Coach</th><th>Students</th><th>Revenue</th></tr>' +
        coachTable +
        '</table>' +
        '<p style="text-align:center;color:#999;margin-top:40px">Generated by Chesskidoo Academy</p>' +
        '</body></html>';
      
      printWindow.document.write(html);
      printWindow.document.close();
      toast('PDF opened - click Print then Save as PDF', 'success');
      return;
    }
