...
<div class="receipt">
  ...
  async function generateReportPDF() {
    ...
    // Header
    doc.setFillColor(220, 163, 62); // Gold
    doc.rect(0, 0, 210, 40, 'F');
    
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(28);
    doc.setFont("helvetica", "bold");
    doc.text('CHESSKIDOO ACADEMY', 20, 25);
    
    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.text('PREMIUM FINANCIAL PERFORMANCE REPORT', 20, 33);
    doc.text(`Report Date: ${dateStr}`, 140, 33);

    // Summary Section
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(18);
    doc.text('Academy Summary', 20, 55);
    ...
    const totalStudents = allStudents.length;
    const paidRevenue = allStudents.filter(s => getStudentPaymentStatus(s) === 'Paid').reduce((a, s) => a + getStudentMonthlyFee(s), 0);
    const dueRevenue = allStudents.filter(s => getStudentPaymentStatus(s) !== 'Paid').reduce((a, s) => a + getStudentMonthlyFee(s), 0);
    ...
    let y = 80;
    const metrics = [
      ['Total Cadets:', totalStudents],
      ['Active Coaches:', allCoaches.length],
      ['Collected Revenue:', `Rs. ${paidRevenue.toLocaleString()}`],
      ['Pending Revenue:', `Rs. ${dueRevenue.toLocaleString()}`],
      ['Total Potential:', `Rs. ${totalPotential.toLocaleString()}`],
      ['Coach Expenses:', `Rs. ${totalCoachCost.toLocaleString()}`],
      ['Net Profit (Current):', `Rs. ${netProfit.toLocaleString()}`]
    ];

    metrics.forEach(m => {
      doc.text(m[0], 25, y);
      doc.text(String(m[1]), 80, y);
      y += 10;
    });
    ...
    y += 10;
    doc.setFontSize(18);
    doc.text('Coach Financial Breakdown', 20, y);
    ...
    doc.text('Revenue', 105, y);
    doc.text('Salary', 135, y);
    ...
    allCoaches.forEach(c => {
      ...
      doc.text(getCoachName(c).substring(0, 25), 20, y);
      doc.text(String(studs.length), 85, y);
      doc.text(rev.toLocaleString(), 105, y);
      doc.text(cost.toLocaleString(), 135, y);
      doc.setTextColor(profit >= 0 ? 0 : 200, profit >= 0 ? 100 : 0, 0);
      doc.text(profit.toLocaleString(), 165, y);
      doc.setTextColor(0, 0, 0);
      y += 8;
    });
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(`Generated by Chesskidoo Admin • Page ${i} of ${pageCount}`, 105, 290, { align: 'center' });
    }
