(function () {
  ...
  // ─── Admin Homework Tracker ──────────────────────────────────────
  let allHomeworkTasks = [];
  let allHomeworkNotes = "";

  window.initHomeworkPage = function() {
      const select = document.getElementById('hw-student-select');
      if (select && window.allStudents) {
          select.innerHTML = '<option value="">-- Choose a Student --</option>';
          const activeStudents = window.allStudents.filter(s => s.status !== 'archived');
  ...
  window.loadAdminHomeworkView = async function() {
      const studentId = document.getElementById('hw-student-select').value;
      if (!studentId) {
          document.getElementById('hw-content-area').style.display = 'none';
          return;
      }
      ...
      try {
          ...
          allHomeworkNotes = notes ? notes.note_text : "";

          document.getElementById('hw-notes-area').value = allHomeworkNotes;
          document.getElementById('hw-content-area').style.display = 'block';
          
          filterAdminHomework();
      } catch (e) {
          console.error("Error loading homework view:", e);
      }
  };

  window.filterAdminHomework = function() {
      const query = document.getElementById('hw-search-input').value.toLowerCase().trim();
      const container = document.getElementById('hw-tasks-container');
      
      container.innerHTML = '';

      if (allHomeworkTasks.length === 0) {
          container.innerHTML = '<div style="color:var(--ivory-dim); font-size:12px;">No tasks assigned yet.</div>';
          return;
      }

      let filteredTasks = allHomeworkTasks;
      if (query) {
          filteredTasks = allHomeworkTasks.filter(t => t.task_text.toLowerCase().includes(query));
      }

      if (filteredTasks.length === 0) {
          container.innerHTML = '<div style="color:var(--ivory-dim); font-size:12px;">No matching tasks found.</div>';
          return;
      }
      ...
      filteredTasks.forEach(t => {
          ...
          const statusIcon = isDone ? '<span style="color:var(--success);">✅ Completed</span>' : '<span style="color:var(--gold);">⏳ Pending</span>';
          const textDeco = isDone ? 'text-decoration:line-through; opacity:0.7;' : '';

          div.innerHTML = `
            <div style="flex:1; font-size:13px; color:var(--ivory); ${textDeco}">
              ${escapeHtml(t.task_text)}
            </div>
            <div style="font-size:11px; white-space:nowrap; border:1px solid rgba(255,255,255,0.1); padding:2px 6px; border-radius:4px; background:rgba(0,0,0,0.2);">
              ${statusIcon}
            </div>
          `;
