...
# Coach name to ID mapping (need to get from DB first)
coach_map = {}
...
for s in students:
    ...
    if r.status_code in [200, 201]:
        ...
        failed.append((s['name'], r.status_code, r.text[:200]))
        print(f"✗ {s['name']}: {r.status_code}")

print(f"\n=== Results ===")
print(f"Successfully inserted: {success_count}/{len(students)}")
if failed:
