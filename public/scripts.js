...
<div class="receipt">
  ...
  async function generateReportPDF() {
    ...
    const ivory = [254, 253, 251]; // Ivory background
    const gold = [179, 133, 27]; // Gold brand color
    const darkGray = [26, 26, 26]; // Ivory/Night text
    ...
    doc.text('CHESSKIDOO ACADEMY', 105, 18, { align: 'center' });
    
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text('PREMIUM FINANCIAL PERFORMANCE REPORT', 105, 26, { align: 'center' });
    
    // Report Date Section
    doc.setTextColor(...darkGray);
    doc.setFontSize(12);
    doc.setFont("helvetica", "bold");
    doc.text('REPORT DATE', 20, 50);
    doc.setFont("helvetica", "normal");
    doc.text(dateStr, 20, 57);
    ...
    const collectionRate = (collected / potential * 100).toFixed(1);
    const profitMargin = (netProfit / (collected || 1) * 100).toFixed(1);
    ...
    // Stats Grid Page 1
    doc.setTextColor(...darkGray);
    const gridX1 = 25, gridX2 = 110, gridYStart = 90, vSpace = 25;
    
    const drawMetric = (x, y, label, val, sub) => {
      doc.setFontSize(9);
      doc.setFont("helvetica", "bold");
      doc.text(label, x, y);
      doc.setFontSize(16);
    ...
    drawMetric(gridX1, gridYStart, 'TOTAL CADETS', `${totalStudents}`, 'Academy Strength');
    drawMetric(gridX2, gridYStart, 'ACTIVE COACHES', `${allCoaches.length}`, 'Teaching Staff');
    ...
    doc.setFontSize(14);
    doc.setTextColor(...gold);
    doc.text('COLLECTION PERFORMANCE', 20, 190);
    doc.line(20, 192, 190, 192);
    
    doc.setFontSize(10);
    doc.setTextColor(...darkGray);
    doc.text(`Collection Rate: ${collectionRate}%`, 25, 205);
    doc.text(`Profit Margin: ${profitMargin}%`, 25, 215);
    
    doc.text('Collected:', 120, 205); doc.text(`Rs. ${collected.toLocaleString()}`, 150, 205);
    doc.text('Pending:', 120, 215); doc.text(`Rs. ${pending.toLocaleString()}`, 150, 215);

    addFooter(1, 3);

    // --- PAGE 2: COACH FINANCIAL BREAKDOWN ---
    doc.addPage();
    doc.setFillColor(...ivory);
    doc.rect(0, 0, 210, 297, 'F');
    ...
    doc.text('Students', 75, 35);
    doc.text('Revenue (Collected)', 100, 35);
    doc.text('Salary Cost', 135, 35);
    doc.text('Net Profit', 160, 35);
    doc.text('ROI', 180, 35);
    ...
    allCoaches.forEach(c => {
      ...
      coachMetrics.push({ name: getCoachName(c), students: studs.length, rev, cost, profit, roi });

      doc.text(getCoachName(c).substring(0, 22), 25, rowY);
      doc.text(String(studs.length), 75, rowY);
      doc.text(`Rs. ${rev.toLocaleString()}`, 100, rowY);
      doc.text(`Rs. ${cost.toLocaleString()}`, 135, rowY);
      doc.text(`Rs. ${profit.toLocaleString()}`, 160, rowY);
      doc.text(`${roi.toFixed(0)}%`, 180, rowY);
    ...
    doc.setFont("helvetica", "bold");
    doc.text('ACADEMY TOTAL', 25, rowY);
    doc.text(String(totalStudents), 75, rowY);
    doc.text(`Rs. ${collected.toLocaleString()}`, 100, rowY);
    doc.text(`Rs. ${payroll.toLocaleString()}`, 135, rowY);
    doc.text(`Rs. ${netProfit.toLocaleString()}`, 160, rowY);
    doc.text(`${(netProfit / (payroll || 1) * 100).toFixed(0)}%`, 180, rowY);
    doc.setFont("helvetica", "normal");
    ...
    doc.setFontSize(9);
    doc.setTextColor(...darkGray);
    doc.setFont("helvetica", "bold"); doc.text('Top Performer:', 25, rowY); doc.setFont("helvetica", "normal");
    doc.text(topCoach ? topCoach.name : 'N/A', 55, rowY);
    if (topCoach) {
      doc.setTextColor(50, 150, 50);
      doc.text(`Rs. ${topCoach.profit.toLocaleString()} net profit with ${topCoach.students} students — highest ROI at ${topCoach.roi.toFixed(0)}%`, 25, rowY + 5);
    }
    
    rowY += 15;
    doc.setTextColor(...darkGray);
    doc.setFont("helvetica", "bold"); doc.text('Attention Required:', 25, rowY); doc.setFont("helvetica", "normal");
    if (lossCoach && lossCoach.profit < 0) {
      doc.setTextColor(...gold);
      doc.text(lossCoach.name, 60, rowY);
      doc.setTextColor(200, 50, 50);
      doc.text(`Rs. ${Math.abs(lossCoach.profit).toLocaleString()} net loss — student payment pending or low batch ROI`, 25, rowY + 5);
    } else {
      doc.text('All coaches profitable.', 25, rowY + 5);
    }
    ...
    let waterY = 35;
    const waterItems = [
      ['Gross Revenue Potential', `+Rs. ${potential.toLocaleString()}`, 'base'],
      ['Collected Fees', `+Rs. ${collected.toLocaleString()}`, 'positive'],
      ['Pending Fees', `Rs. ${pending.toLocaleString()}`, 'neutral'],
      ['Total Coach Payroll', `-Rs. ${payroll.toLocaleString()}`, 'negative'],
      ['Net Operating Profit', `+Rs. ${netProfit.toLocaleString()}`, 'positive_bold']
    ];
    
    waterItems.forEach(item => {
      doc.text(item[0], 25, waterY);
      if (item[2].includes('bold')) doc.setFont("helvetica", "bold");
      if (item[2].includes('negative')) doc.setTextColor(200, 50, 50);
      if (item[2].includes('positive')) doc.setTextColor(50, 150, 50);
    ...
    recommendations.forEach(rec => {
      doc.setFillColor(...gold);
      doc.rect(20, waterY - 5, 8, 8, 'F');
      doc.setTextColor(255, 255, 255);
      doc.text(rec[0], 24, waterY, { align: 'center' });
      
      doc.setTextColor(...gold);
      doc.setFont("helvetica", "bold");
      doc.text(rec[1], 32, waterY);
      
      doc.setTextColor(...darkGray);
      doc.setFont("helvetica", "normal");
      doc.setFontSize(9);
      const lines = doc.splitTextToSize(rec[2], 150);
      doc.text(lines, 32, waterY + 6);
    ...
    doc.setFontSize(12);
    doc.setTextColor(...gold);
    doc.text('CHESSKIDOO ACADEMY — Premium Chess Education', 105, 260, { align: 'center' });
    doc.setFontSize(9);
    doc.setTextColor(150, 150, 150);
    doc.text(`Generated: ${dateStr} • Confidential`, 105, 267, { align: 'center' });
