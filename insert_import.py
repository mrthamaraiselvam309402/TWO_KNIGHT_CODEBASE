...
BASE_URL = "https://vseombfkrvpffnpgbsnk.supabase.co/rest/v1/website_students_import"
...
print("Inserting 45 students into website_students_import...")
...
for student in students:
    ...
    try:
        ...
        if resp.status_code in [200, 201]:
            success += 1
            print(f"[{success}] {student['name']}")
        else:
            print(f"Error {student['name']}: {resp.status_code}")
    except Exception as e:
        print(f"Exception {student['name']}: {e}")

print(f"\n=== DONE: {success} records inserted ===")

# Verify
final_resp = requests.get(BASE_URL, headers=headers)
print(f"Final count in website_students_import: {len(final_resp.json())}")